...
namespace MINISOCKS {
    ...
    int init () {
        ...
        if(false) {
            ...
            if(stat(config, &sbuf))
                return -1;
            if (checktime == sbuf.st_mtime and not strcmp(config, config_sav))
                return 0;
...
int connect (int sd,  const struct sockaddr *name, socklen_t namelen) {
    ...
    if(name->sa_family == AF_INET) {
        memcpy(pkt0.pkt.dstport, &((sockaddr_in*) name)->sin_port,
                sizeof(pkt0.pkt.dstport));
        memcpy(pkt0.pkt.dstip, &((sockaddr_in*) name)->sin_addr.s_addr,
    ...
    if (NULL == (srv = MINISOCKS::getserver())) {
        errno = EFAULT;
        return -1;
    }
